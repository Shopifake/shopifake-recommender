...
├── config.py            # Environment + Qdrant settings
├── models/product.py    # Pydantic schemas for incoming payloads
└── services/
    ├── clients/         # External API clients (OpenAI encoder/decoder)
    ├── core/            # Core business logic (currently empty)
    ├── queue/           # Redis stream and DLQ management
